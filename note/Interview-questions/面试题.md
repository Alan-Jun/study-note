...
**不管是 RDB 还是 AOF 都是为了机器down机后为了恢复数据使用的**, **除此之外在主从数据同步中也会使用放到**
...
1. AOF持久化的速度，相对于RDB较慢，存储的是一个文本文件，到了后期文件会比较大，传输困难。
...
* 根据机器的实际处理能力（**自适应的负载均衡算法**）
  * [最短响应优先 + 加权随机](#最短响应优先 + 加权随机) : 统计一个一个服务器一个时间窗口的平均耗时，耗时越短的越优先调用
...
**缺点：可能因为线程调度问题导致 令牌生成速度不符合我们配置的要求**
...
处理这种方式还可以使用时间的方式计算里面存入的令牌, **这个可以避免由于线程调度的问题，不满足我们的令牌生成速率**
...
我们来看一下 reddission  具体的实现代码： **看完后你也能知道为什么没有使用。 setNX 来做，因为setNX 不方便实现锁的重入**
