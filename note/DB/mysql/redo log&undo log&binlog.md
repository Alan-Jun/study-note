...
![image-20200312095710182](assets/image-20200312095710182.png)



​	**我们的数据更新过程中在server层完成binlog的记录，再InnoDB层完成它特有的redo log以及undo log的记录**

![DMl语句执行流程&日志记录整体过程](assets/DMl语句执行流程&日志记录整体过程.jpg)
...
首先我们来想想，在一次事务中，如果我存在4个DML语句，都会涉及到修改数据，那么如果我每一次修改数据都要立刻持久化到磁盘，意味着需要执行4词IO操作，那么作为一个高性能数据库，大量的调用这样的IO操作，必然会成为性能瓶颈，那么我们就需要解决这样的问题，思路：
