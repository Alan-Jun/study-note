...
并发安全其实实质是内存的安全。或则说是共享存储区域的数据的安全，我们知道简单的查询其实并不会有什么问题，最多就是查询到旧数据，多查几次也就对了，所以我们主要讨论对这个区域的修改的问题，然后**由问题道出并发安全的定义**：

1. 覆盖操作

   不管是单机还是多进程条件下都是直接将一个值直接覆盖掉共享内存中原有的值

   覆盖操作如果在我们的主观意愿中没有顺序要求那么是不存在什么并发安全问题的

2. 读改写操作

   1. 在单机上我们的操作本质是由 cpu操作的，他是需要先将数据从共享内存中取出来放在cpu的操作内存中然后修改完数据之后在同步回去
   2. 在多进程（分布式）场景也是需要先读到一个数据，然后操作完成之后同步到共享的内存区域

   而读改写问题才是并发安全问题最主要需要面对的，因为我们必定面临读去共享数据，修改，同步这样的三步，而在并发情况下就会面临几个问题

   * **可见性**：我们无法感知到共享数据的变化

     * 多进程因为数据读取到不同的机器之后，他们之间是无法知道对方数据做了什么修改的

     * 多线程

       * 单核心，他们是做的时间片切换，共享数据的操作也是在他们的私有内存中的

       * 多核心，也是保存在线程私有内存中操作的

         > 当然也可以采用cpu间共享一块内存的方式存储，然后并行操作的时候还是会有问题，一开始都是50，并行修改+1，那么数据结果是51，但是正确的结果应该是52 ，从这里也能看出解决可见性问题并不能解决并发安全问题

     如果我们采取一些措施解决可见性问题，让我们能够看到其他线程/进程对这个数据的修改，能解决这个共享内存的安全性问题吗？

     答案是：no, 因为就算能够看到这个数据的变化，但是如果不是并发，而是并行呢？两个线程/进程（或则是多个）同时读取到数据都是50，他们都想给数据+1 ，然后同时去同步他们操作之后的结果，那么是不是还是存在问题呢？ 这就引出来了我们面临的另一个问题**顺序性问题**

   * **顺序性**：上面我们说到了可见性并不能解决并发安全问题，但是单纯的顺序性能够解决并发安全问题吗？假如现在我们对这个共享数据的操作是有顺序的，A先来读取数据，cpu操作+1，写会线程的私有内存。下一个线程B在此重复操作，但是它读取到的数据还是50，由于没有保证可见性，它看不到线程A的操作结果。

     所以我们单纯的保证顺序性也是无法解决并发导致的共享内存的安全性问题的，但是这就足够了吗

     我们都知道我们cpu执行任务的时候是使用时间片来做切换的，假如我们现在解决了可见性和顺序性问题，然后我们再来看一个情况：

     A线程开始操作，读取 a=50, a++, set a ,但是这时候在执行到a++ 之后线程切换了，B线程开始执行，顺序性：我们保证A先于B执行，执行顺序确实是A先于B, 保证A的修改对B可见，但是A这时候还没有set A，也就是它的修改还没完成，所以B这时候是看不到A的操作的。这时候B还是只能读取到a=50，然后执行后续的a++,set a 操作

     发现问题了吗？并发安全问题还是存在，为什么呢？因为线程的执行是通过抢占cpu的时间片来操作的（这样设计的原因是因为cpu资源很宝贵，然后为了让用户在使用电脑的时候感知上是多个程序在并行执行的，所以是采用的分配cpu时间片的方式在运行 ），而乐死读改写这种操作它在cpu执行时不是原子操作，所以会存在这样的问题

     **结论：我们仅仅依靠可见性，顺序性是不足以保证并发安全的，还需要原子性**

   * **原子性**：原子性，即一个操作或多个操作，要么全部执行并且在执行的过程中不被打断，要么全部不执行。
...
* **也可使用这个方法：setUncaughtExceptionHandler 设置**

# 9 FutureTask

https://blog.csdn.net/u012796139/article/details/109348850

FutureTask 源码不看 其他同学分享的博客其实也很好读懂，可以直接看源码的
