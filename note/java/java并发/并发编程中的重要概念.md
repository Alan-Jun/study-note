...
并发安全其实实质是内存的安全。或则说是对共享存储区域的数据修改的安全，我们知道简单的查询其实并不会有什么问题，最多就是查询到旧数据，多查几次也就对了，所以我们主要讨论对这个区域的修改的问题，然后**由问题导出并发安全的定义**：
...
2. 读改写操作
   ...
   * **顺序性**：上面我们说到了可见性并不能解决并发安全问题，但是单纯的顺序性能够解决并发安全问题吗？假如现在我们对这个共享数据的操作是有顺序的，A先来读取数据，cpu操作+1，写会线程的私有内存。下一个线程B在此重复操作，但是它读取到的数据还是50，由于没有保证可见性，它看不到线程A的操作结果。
     ...
     发现问题了吗？并发安全问题还是存在，为什么呢？因为线程的执行是通过抢占cpu的时间片来操作的（这样设计的原因是因为cpu资源很宝贵，然后为了让用户在使用电脑的时候感知上是多个程序在并行执行的，所以是采用的分配cpu时间片的方式在运行 ），而类似读改写这种操作它在cpu执行时不是原子操作，所以会存在这样的问题
