...
总结一下：链表也是一种**线性表**数据结构。相比数组（Array）链表**没有要求存储的数据是在一组连续的内存空间**，**链表也不会要求链表中的数据类型要相同**。链表中的元素间关系是通过其中的元素自身存储和他有关联的元素的内存地址来实现的，一个链表中的元素正式靠着这样的关系将他们链接到同一个链表上的
...
有利就有弊，链表在查询的时候希望访问第k个元素就没有数组那么高效了，因为它不像数组一样内存连续，其中元素类型也相同，只需要通过首地址和下标就能结算得到元素的内存地址，链表则需要一个结点一个结点的依次便利，直到找到相应的结点
...
如果是类似 删除结点中“值等于某个给定值”的结点；这样的问题单向双向都需要从头开始便利去找，所以双向链表在这类问题上没有任何优势，但是如果是删除给定指针指向的结点。我们在删除结点的时候需要找到他的前置结点，这时候双向链表的优势就来了，他只需要O(1)的时间复杂度就能获取到他的前置结点（前置结点 = currentNode.pre）；同理如果我们希望给丁的节点前面插入一个结点，双向链表也更有优势，当然还有诸如查找链表中第k个结点这样的操作，在我们记录了链表长度的情况下，双向链表可以选择最短路径所以也更有优势... 
...
这种实现方式不管我们缓存满还是未满我们都需要便利链表查询元素是否已经在链表中所以实现复杂度时O(n),
...
3. 在插入数据的时候和链表的也类似，需要先便利看数据是否已经在数组中存在：
