...
![img](assets/v2-ecd389d78d54504eb3e4c9c740edf1ee_720w.jpg)
...
![img](assets/v2-7ef4dd23b909affc4c5529bad4b0564b_720w.png)
...
![img](assets/v2-fde2bdeda023c56b686dccaa1da1b513_720w.png)
...
![img](assets/v2-ae0048a2396be9f73caf83bb347c37fc_720w.png)
...
![img](assets/v2-6ad164e00b151826f11559159b8276a2_720w.png)
...
L**inux 2.6.32内核之后，放弃了原有的pdflush机制，改成了bdi_writeback机制。bdi_writeback机制主要解决了原有fdflush机制存在的一个问题：在多磁盘的系统中，pdflush管理了所有磁盘的Cache，从而导致一定程度的I/O瓶颈。bdi_writeback机制为每个磁盘都创建了一个线程，专门负责这个磁盘的Page Cache的刷新工作，从而实现了每个磁盘的数据刷新在线程级的分离，提高了I/O性能。**

**回写机制存在的问题是回写不及时引发数据丢失（可由sync|fsync解决），回写期间读I/O性能很差。 **

**合理利用Page Cache 的预读/回写机制可以大大的提高性能，比如Kafka就利用了这一点**

> mysql的设计实现中（比如 redo log的数据读/写）就是用了这里的page Cache 特性，以及 fsync ,sync方法
...
![img](assets/v2-7afcfcc4d05c824bc2a1501f962108f1_720w.png)
...
![img](assets/v2-4ddcd8e6acd243dacf87ada63a92703d_720w.png)
...
![img](assets/v2-b5e8344a51cb1ea10fd6f70cde99dc29_720w.png)
...
> 来自：https://blog.csdn.net/weixin_36145588/article/details/78430233